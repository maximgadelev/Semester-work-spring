...
public interface TripRepository extends JpaRepository<Trip, Integer> {
    ...
    @Query(value = "select *from (select p.id,c.id as car_id  from passengers p inner join car c on p.id = c.driver_id) as driver_car inner join trip t on t.car_id=driver_car.car_id where status = ? and driver_car.id= ?",nativeQuery = true)
